...
MODELS_TO_EXCLUDE = [
    'Gateways', 'Budget', 'ContentTypes', 'Dlp', 'PromptSafety', 'ContentFilter',
    'Model', 'Routes', 'Secrets', 'Message', 'Usage', 'Choice', 'QueryResponse',
    'JavelinConfig', 'HttpMethod', 'Request', 'ChatCompletion', 'ResponseMessage', 'APIKey'
]
...
def modify_and_convert_swagger(input_file, output_file):
    ...
    if '/v1/admin/providers/secrets/keys' in swagger_data['paths']:
        ...
        for method in path.values():
            if 'parameters' in method:
                method['parameters'] = [param for param in method['parameters'] if param.get('name') != 'providername']
    ...
    if '/v1/admin/dataprotection/templates' in swagger_data['paths']:
        ...
        for method in path.values():
            if 'parameters' in method:
                method['parameters'] = [param for param in method['parameters'] if param.get('name') != 'templatename']
    ...
    if response.status_code == 200:
        ...
        print(f"Error converting to OpenAPI 3.0: {response.status_code} - {response.text}")
