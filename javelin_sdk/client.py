    ...
    def register_provider(self, 
        ...
        def _capture_response_details(span, response, kwargs, system_name):
            try:
                ...
                response_type = response_data.get("object", "")

                # **Handle Embeddings (`object == "list"`)**
                if response_type == "list" and isinstance(response_data.get("data"), list):
                    JavelinClient.set_span_attribute_if_not_none(span, gen_ai_attributes.GEN_AI_PROMPT, kwargs.get("input"))
                    
                    embedding_data = response_data["data"]  # List of embeddings
                    JavelinClient.set_span_attribute_if_not_none(span, gen_ai_attributes.GEN_AI_COMPLETION, json.dumps(embedding_data))
                    
                    return  # Stop further processing since embeddings don’t have choices

                # **Handle Chat Completion (`object == "chat.completion"`)**
                # if response_type == "chat.completion" or response_type == "completion":
                if (response_type == "chat.completion" or response_type == "completion") or "choices" in response_data:
                    ...
                    for key in input_keys:
                        if key in kwargs:
                            ...
                            if key == "messages" and isinstance(value, list):  
                                input_value = value
                            elif key in ["input", "prompt"] and isinstance(value, str):  
                                input_value = [value]  
                            break  

                    JavelinClient.set_span_attribute_if_not_none(span, gen_ai_attributes.GEN_AI_PROMPT, json.dumps(input_value))
                    ...
                    for key in output_keys:
                        if key in response_data and isinstance(response_data[key], list):
                            output_value = response_data[key]
                            break  

                    JavelinClient.set_span_attribute_if_not_none(span, gen_ai_attributes.GEN_AI_COMPLETION, json.dumps(output_value))
                ...
                span.set_attribute("javelin.response.error", str(e))
